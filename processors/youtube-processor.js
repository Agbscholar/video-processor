...
// Enhanced downloadWithYtDlp method with cookie support
async downloadWithYtDlp(videoUrl, processingId) {
  const outputTemplate = path.join(this.tempDir, `${processingId}_original.%(ext)s`);
  const profile = this.getCurrentProfile();
  
  // Cookie file path (you'll need to implement cookie extraction)
  const cookiePath = process.env.YOUTUBE_COOKIES_PATH || '/app/cookies/youtube_cookies.txt';
  ...
  const options = [
    ...
    ...(fsSync.existsSync(cookiePath) ? ['--cookies', cookiePath] : []),
...
// Enhanced ytdl-core method with better options
...
// Add these new methods to your YouTubeProcessor class

// Method using Invidious instances as proxy
async downloadViaInvidious(videoUrl, processingId) {
  ...
  for (const instance of invidiousInstances) {
    try {
      ...
      const infoResponse = await axios.get(`${instance}/api/v1/videos/${videoId}`, {
        timeout: 30000,
        headers: {
          'User-Agent': this.getCurrentProfile().userAgent
        }
      ...
      const stream = videoInfo.formatStreams
        .filter(s => s.container === 'mp4' && s.qualityLabel)
        .sort((a, b) => parseInt(b.qualityLabel) - parseInt(a.qualityLabel))[0];

      if (!stream) {
        throw new Error('No suitable MP4 stream found');
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
          writeStream.destroy();
          reject(new Error('Invidious download timeout'));
        }, 600000);
      });

    } catch (error) {
      console.log(`[${processingId}] Invidious instance ${instance} failed: ${error.message}`);
      continue;
    }
  }
...
// Method using direct YouTube API URLs with mobile client
async downloadViaMobileClient(videoUrl, processingId) {
  const videoId = this.extractVideoId(videoUrl);
  if (!videoId) {
    throw new Error('Cannot extract video ID');
  }
  ...
  try {
    // Use mobile client parameters
    const apiUrl = 'https://www.youtube.com/youtubei/v1/player';
    const params = {
      key: 'AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8', // Public YouTube web client key
      contentCheckOk: true,
      racyCheckOk: true
    };
    ...
    const response = await axios.post(`${apiUrl}?${new URLSearchParams(params)}`, data, {
      headers: {
    ...
    if (!response.data.streamingData?.formats && !response.data.streamingData?.adaptiveFormats) {
    ...
    return new Promise((resolve, reject) => {
      writeStream.on('finish', () => {
        console.log(`[${processingId}] Mobile client download completed`);
        resolve(outputPath);
      });
      ...
      setTimeout(() => {
        writeStream.destroy();
        reject(new Error('Mobile client download timeout'));
      }, 600000);
    });

  } catch (error) {
    throw new Error(`Mobile client download failed: ${error.message}`);
  }
}

// Enhanced download method with all alternatives
async downloadVideoSafe(videoUrl, processingId) {
  await this.enforceAdvancedRateLimit();
  ...
  const methods = [
    ...
    () => this.downloadViaMobileClient(videoUrl, processingId),
    () => this.downloadWithYtdlCore(videoUrl, processingId)
  ];
